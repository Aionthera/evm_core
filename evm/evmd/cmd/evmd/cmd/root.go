...
func initCometConfig() *cmtcfg.Config {
	...
	// The EVM module ships its own app-side mempool; CometBFT must delegate
	// mempool management to the application instead of its built-in gossip
	// mempool, or `start` fails with "EVM mempool enabled, but comet-bft has
	// invalid config.toml:mempool.type".
